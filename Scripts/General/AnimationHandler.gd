...
func Update():
	...
	if not player_sprite or not is_instance_valid(player_sprite):
		return
	
	if not _effectsPlayer or not is_instance_valid(_effectsPlayer):
		return
	
	var effects_parent = _effectsPlayer.get_parent()
	if not effects_parent or not is_instance_valid(effects_parent):
		return
	
	if(effects_parent.z_index != player_sprite.z_index + 1):
		effects_parent.z_index = player_sprite.z_index + 1
